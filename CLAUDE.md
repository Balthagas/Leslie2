...
Lean 4 + Mathlib formalization project. The repo is named "leslie2" externally (GitHub, blueprint) but the Lake package and root module are `MyMathlibProject` — the rename was not propagated. The mathematical content — probabilistic labelled transition systems (`PLTS`) and the trace-distribution soundness of probabilistic simulations — is organized into thematic folders under `MyMathlibProject/` (`Util/`, `Model/`, `Weak/`, `Construction/`, `Expansion/`, `Simulation/`); `MyMathlibProject.lean` is the module root that re-exports every file. See `README.md` for the full layout, the dependency layers, and where each main result lives. The PLTS core model is in `MyMathlibProject/Model/System.lean`.
...
lake build MyMathlibProject.Model.System  # build a single file
