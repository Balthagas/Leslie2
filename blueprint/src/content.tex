...
\blueprintfig{fig-ts1-rules}{Transition System 1, the repaired ABA specification: the three control states \emph{unbound} ($bind = \bot$), \emph{bound} ($bind \neq \bot$, $val = \bot$) and \emph{decided} ($val$ set), with rules 1--8 as labelled transitions.
The diagram draws each rule once; enabledness is wider: rule 2 fires in every state, rule 4 also re-binds from \emph{bound}, and rules 3, 5 and 6 remain enabled on \emph{decided}, where rule 3 rewrites $val$ to itself.
...
Rule 9 (\texttt{fail}), the total corruption rule (D1), and rule 10 (\texttt{callByzFill}), by which a corrupted $id\in F$ fills an empty call slot with no ghost record (D13), are omitted from the diagram.
