\begin{theorem}[Soundness of probabilistic forward simulation]
  ...
  If $sys_C$ is forward-simulated by $sys_A$ (along some $R : State_C \to \operatorname{PMF} State_A \to \mathrm{Prop}$), then
  \[
...
\begin{proof}\leanok
  ...
  Chain the four building blocks.
  By Theorem~\ref{thm:forward-equiv} the forward simulation $R$ is a strong simulation of $sys_C$ by $\mathcal{D}(sys_A^w)$, so Theorem~\ref{thm:strongsound} gives
  \[
    \operatorname{ATD}(sys_C)
      \subseteq \operatorname{ATD}(\mathcal{D}(sys_A^w))
      = \operatorname{ATD}(sys_A^w)
      = \operatorname{ATD}(sys_A),
  \]
  the first equality by Theorem~\ref{thm:dist-eq} at $sys_A^w$ and the second by Theorem~\ref{thm:weakclosure-eq} at $sys_A$.
\end{proof}
