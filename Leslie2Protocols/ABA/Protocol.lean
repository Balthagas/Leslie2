...
* **D20 (instance forgetting).** A process record carries one graded-agreement
...
multicast. A process record carries the stage record of one round only, so
every stage-side row is guarded by
