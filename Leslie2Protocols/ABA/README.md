...
| `GBCAFamily.lean` | 130 | Lifts the per-instance refinement to the ℕ-indexed **family** (`familyRefines`) via the framework's family congruence. What the chain draws from here is `instRel_corrupt`, the compatibility of the per-instance relation with the broadcast `fail` act, which the round subsystem's family lifting consumes. |
