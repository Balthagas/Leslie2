...
| `GBCAFamily.lean` | 143 | Lifts the per-instance refinement to the ℕ-indexed **family** (`familyRefines`) via the framework's family congruence. |
...
| `CoreProc.lean` | 1258 | The same for the core: `CoreNode` (control record, DECIDED pool, inbox rows, `F` copy), gossip as `net(i, j, b)` rendezvous, and **`perProcCore_atd`**. The per-(receiver, sender, bit) DECIDED pools (D12′) are what let the delivery guard split sender/receiver-locally. |
...
| `FlatOwnFlag.lean` | 1434 | **The deployed reading of the flat presentation**: each process carries one Boolean corruption flag of its own and no copy of the corrupted set (`GBCA.ProcNodeU`, `CoreNodeU`, `ABANodeU = CoreNodeU × (ℕ → ProcNodeU) × Bool`), and `fail k` raises `k`'s flag with no guard and no budget in the state. `OwnFlag.ABAProcStepU` is `Flat.ABAProcStep` with exactly three deltas — the record shape, every Byzantine guard `j ∈ F` becoming `flag = true`, and the unguarded idempotent flag write in place of the budget-guarded insert into every copy — so the honest rules port verbatim, none of them reading `F`. `ownFlagFlat` mirrors `Flat.flatHybrid` shape for shape, the coin oracle entering unchanged with its own copy of the corrupted set and its budget-guarded `corrupt`. **The conservativity bridge**: `inflate` reads an own-flag state as a guarded flat state, every deleted copy of the corrupted set reconstructed as the one global `flagSet`; `ownFlagFlatB_bridge` matches every step of the budgeted system `ownFlagFlatB` — each `fail` a repeat or fired with headroom — along it, and the trace-level budget `BudgetTrace f t`, that the `fail` labels of `t` name at most `f` ids, closes the gap to the unrestricted system: every `fail` of a witness execution is exposed in its trace, so the flag set stays inside the trace's fail set and pruning the scheduler's out-of-budget emissions (`pruneSched`) leaves that execution's probability alone. Headlines: `ownFlagFlat_safe` (every positive-probability trace respecting the budget is valid and agreeing) and `ownFlagFlat_traces` (it is a trace of `hybridImpl`), both by composition with `FlatABA`'s pair, with a `#guard_msgs` axiom firewall on each. |
...
- **Budget as an assumption throughout** (banked): unguarded `fail` in every system,
  ...
  `BudgetTrace` — a full-replacement campaign. `FlatOwnFlag`'s bridge is the
