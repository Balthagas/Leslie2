...
| `GBCAFamily.lean` | 84 | `instRel_corrupt`, the compatibility of the per-instance relation with the broadcast `fail` act, proved directly rather than through `implRefines`; the round subsystem's family lifting consumes it, and it is where `ForwardSimulation.family` (`Framework/FamilySim.lean`) is first applied. |
